...
async def log_requests(request: Request, call_next):
    ...
    logger.info("Request started - ID: %s, Method: %s, URL: %s", request_id, request.method, str(request.url))
    
    response = await call_next(request)
    
    process_time = time.time() - start_time
    logger.info("Request completed - ID: %s, Status: %s, Time: %.3fs", request_id, response.status_code, process_time)
...
async def health_check():
    ...
    try:
        ...
        return {
            ...
            "services": {
                "mongodb": "connected",
                "vector_db": "connected" if hasattr(app, 'vectordb_client') else "not_initialized",
